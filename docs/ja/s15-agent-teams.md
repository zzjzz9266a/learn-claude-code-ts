...
class TeammateManager {
    ...
    constructor(team_dir: Path) {
        this.team_dir = team_dir;
        this.config_path = team_dir.join("config.json");
...
spawn(name: string, role: string, prompt: string): void {
    ...
    // In TypeScript/Node.js, use worker threads or async functions
    // This is conceptual translation
    this._teammate_loop(name, role, prompt);
}
```
...
send(sender: string, to: string, content: string): void {
    ...
    // fs.appendFileSync(`${to}.jsonl`, JSON.stringify(record) + "\n");
}
```
...
```typescript
function teammate_loop(name: string, role: string, prompt: string): void {
